...
const Auth = () => {
  const { user, loading, signIn, unauthorized } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast({
        title: "Errore",
        description: err.message || "Errore durante l'accesso con Google.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
        ...
        <CardContent className="space-y-6">
          {unauthorized && (
            <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive text-center">
              Accesso non autorizzato. Il tuo account non è abilitato.
            </div>
          )}
          ...
          {mode === "cloud" ? (
            /* Cloud mode: Google sign-in only */
            <Button
              type="button"
              variant="outline"
            ...
            >
              ...
              {submitting ? "Caricamento..." : "Accedi con Google"}
            </Button>
          ) : (
